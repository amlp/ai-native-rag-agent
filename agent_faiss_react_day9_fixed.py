...
# 4. FAISS混合检索RAG工具 修复重复文档、清空向量库、结果去重
...
# 6. ReAct Agent 支持多需求自动分轮调度工具，强化无关文档判断规则
...
5. 若doc_search返回的知识库内容和用户问题完全无关，等同于工具失败，禁止使用该文档推理；
6. 完成检索后缺少计算所需用户参数（工龄、年限等），禁止调用计算器，直接向用户提问获取缺失信息；
输出格式：
需要工具：Thought:思考内容\nAction:工具名|参数
